...
const Canvas = () => {
  ...
  const syncLayers = useCallback((canvas) => {
    ...
    setLayers([...layers].reverse()); // top-most first
  }, [setLayers]);
 
  // ─── Save canvas state to history ───────────────────────────────────────────
  const saveHistory = useCallback((canvas) => {
    if (!useWhiteboardStore.getState().isReplaying) {
      pushHistory(JSON.stringify(canvas.toJSON(['id', 'customName', 'customType'])));
    }
  }, [pushHistory]);
  ...
  useEffect(() => {
    ...
    fabricCanvas.on('mouse:up', () => {
      if (isPanningRef.current) {
        ...
        fabricCanvas.selection     = useWhiteboardStore.getState().activeTool === 'select';
    ...
    const onKeyDown = (e) => {
      ...
      // Tool shortcuts
      const shortcuts = { v: 'select', p: 'pen', s: 'shape', t: 'text', n: 'sticky', e: 'eraser' };
      if (document.activeElement.tagName !== 'INPUT' && shortcuts[e.key]) {
        store.setActiveTool(shortcuts[e.key]);
      }
    };
  ...
  useEffect(() => {
    ...
    switch (activeTool) {
      ...
      case 'shape': {
        ...
        mdHandler = (opt) => {
          ...
          const base = {
            ...
            id: uid('shape'), customName: activeShape,
          };
          ...
          switch (activeShape) {
            ...
            case 'line':
              shape = new fabric.Line(
                ...
                { stroke: STROKE, strokeWidth: STROKE_W, selectable: false, evented: false, id: uid('line'), customName: 'Line' }
              );
              break;
            case 'arrow':
              // Arrow is created fully on mouse:up
              shape = null;
              break;
            case 'diamond':
              ...
              break;
            default: // rect
              shape = new fabric.Rect({ ...base, width: 0, height: 0, rx: 3, ry: 3 });
          }
        ...
        mmHandler = (opt) => {
          ...
          switch (activeShape) {
            ...
            case 'arrow':
              // preview – no shape until mouse:up
              break;
            default:
        ...
        muHandler = (opt) => {
          ...
          if (activeShape === 'arrow') {
            const arrow = createArrowGroup(start.x, start.y, ptr.x, ptr.y, STROKE, STROKE_W);
            arrow.id         = uid('arrow');
      ...
      case 'text': {
        ...
        mdHandler = (opt) => {
          if (isPanningRef.current || opt.e.altKey) return;
          // If clicking on an existing IText, let fabric handle it
          ...
          // On exiting editing save history
    ...
    return () => {
      // Clean up only the tool-specific handlers
  ...
  }, [activeTool, activeShape, penColor, penWidth, syncLayers, saveHistory]);
